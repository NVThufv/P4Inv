  ...
  public class SMTLibInteractiveTheoremProver : SMTLibProcessTheoremProver
  {
    ...
    public override async Task<Outcome> Check(string descriptiveName, VCExpr vc, ErrorHandler handler, int errorLimit, CancellationToken cancellationToken)
    {
      ...
      try
      {
        ...
        SendThisVC("(push 1)");
        DeclCollector.Push();
        // Console.WriteLine("---- VC: {0}",  VCExpr2String(vc, 1));
      ...
      while (true)
      {
        ...
        try
        {
          ...
          if (result == Outcome.Invalid)
          {
            ...
            await currentErrorHandler.OnModel(labels, model, result);
          }
    ...
    public override async Task<object> Evaluate(VCExpr expr)
    {
      ...
      // Console.WriteLine($"VCExpr: {expr}");
      // Console.WriteLine($"vcString: {resp}");
      if (resp == null)
      {
      ...
      if (resp.ArgCount != 0)
      {
        String parse_result = "false";
        return bool.Parse(parse_result);
      }

      if (expr.Type.Equals(Boogie.Type.Bool))
      {
        // Console.WriteLine("Bool: resp: " + resp);
        // Console.WriteLine("bool resp name: " + resp.Name);
