...
namespace VC
{
  ...
  public class VCGen : ConditionGeneration
  {
    ...
    public VCExpr P_GenerateVC(Implementation /*!*/ impl, HashSet<string> constantsAssumed, VCExpr controlFlowVariableExpr,
      ControlFlowIdMap<Absy> absyIds, ProverContext proverContext)
    {
      Contract.Requires(impl != null);
      Contract.Requires(proverContext != null);
      ...
      return P_GenerateVCAux(impl, constantsAssumed, controlFlowVariableExpr, absyIds, proverContext);
    }
    ...
    public VCExpr P_GenerateVCAux(Implementation /*!*/ impl, HashSet<string> constantsAssumed, VCExpr controlFlowVariableExpr,
      ControlFlowIdMap<Absy> /*!*/ absyIds, ProverContext proverContext)
    {
      Contract.Requires(impl != null);
      Contract.Requires(proverContext != null);
      ...
      if (cce.NonNull(CheckerPool.Options.TheProverFactory).SupportsDags)
      {
        vc = P_DagVC(cce.NonNull(impl.Blocks[0]), constantsAssumed, controlFlowVariableExpr, absyIds,
          new Dictionary<Block, VCExpr>(), proverContext, out assertionCount);
      }
      else
      {
        vc = P_LetVC(impl.Blocks, constantsAssumed, controlFlowVariableExpr, absyIds, proverContext, out assertionCount);
      }
      ...
      if (impl.RecycledFailingAssertions != null && impl.RecycledFailingAssertions.Any())
      {
        ...
        foreach (var a in impl.RecycledFailingAssertions)
        {
          ...
          if (impl.ErrorChecksumToCachedError[checksum] is Counterexample oldCex)
          {
            ...
            {
              await callback.OnCounterexample(oldCex, null);
            }
            else
            {
              ...
              await callback.OnCounterexample(cex, null);
            }
          }
        }
      }
    ...
    public class P_ErrorReporter : ProverInterface.ErrorHandler {
      ...
      public P_ErrorReporter(VCGenOptions options, HashSet<string> constantsAssumed,
        Dictionary<TransferCmd, ReturnCmd> /*!*/ gotoCmdOrigins,
        ControlFlowIdMap<Absy> /*!*/ absyIds,
        List<Block /*!*/> /*!*/ blocks,
      ...
      public async override Task OnModel(IList<string> labels /*!*/ /*!*/, Model model,
        ProverInterface.Outcome proverOutcome)
      {
        ...
        // Console.WriteLine("Entry: {0}", entryBlock);
        // foreach(var tn in traceNodes)
        // {
        //   Console.WriteLine("TreeNode: {0}", tn);
        // }
        ...
        Counterexample newCounterexample = P_TraceCounterexample(options, assumedCmds, constantsAssumed, entryBlock, traceNodes, trace, model, MvInfo,
        ...
        // Console.WriteLine("P_ErrorReporter: OnCounterexample Begining");
        // await callback.OnCounterexample(newCounterexample, null);
        await callback.OnCounterexample(newCounterexample, null);
      ...
      public override async Task OnResourceExceeded(string msg, IEnumerable<Tuple<AssertCmd, TransferCmd>> assertCmds = null)
      {
        ...
        if (assertCmds != null)
        {
          foreach (var cmd in assertCmds)
          {
            ...
            await callback.OnCounterexample(cex, msg);
          }
        }
      }
    ...
    public class ErrorReporter : ProverInterface.ErrorHandler {
      ...
      public override async Task OnModel(IList<string> labels /*!*/ /*!*/, Model model,
        ProverInterface.Outcome proverOutcome)
      {
        // no counter examples reported.
        if (labels.Count == 0)
        ...
        Counterexample newCounterexample = TraceCounterexample(options, assumedCmds, entryBlock, traceNodes, trace, model, MvInfo,
          debugInfos, context, split, new Dictionary<TraceLocation, CalleeCounterexampleInfo>());
...
//      #endregion
    ...
    static Counterexample P_TraceCounterexample(
      VCGenOptions options, List<Cmd> /*!*/ assumedCmds, HashSet<string> constantsAssumed,
        ...
        for (int i = 0; i < cmds.Count; i++)
        {
          ...
          if (cmd is AssertCmd && traceNodes.Contains(cmd))
          {
            Counterexample newCounterexample =
              AssertCmdToCounterexample(options, assumedCmds, (AssertCmd) cmd, transferCmd, trace, augmentedTrace, errModel, mvInfo, context, split);
    ...
    static Counterexample TraceCounterexample(
      VCGenOptions options, List<Cmd> /*!*/ assumedCmds,
      Block b, HashSet<Absy> traceNodes, List<Block> trace, Model errModel, ModelViewInfo mvInfo,
      Dictionary<Cmd, List<object>> debugInfos,
      ProverContext context,
      ProofRun split,
    ...
    public static Counterexample AssertCmdToCounterexample(VCGenOptions options, List<Cmd> assumedCmds, AssertCmd cmd, TransferCmd transferCmd, List<Block> trace, List<object> augmentedTrace,
      Model errModel, ModelViewInfo mvInfo, ProverContext context, ProofRun split)
    {
      Contract.Requires(cmd != null);
      Contract.Requires(transferCmd != null);
      ...
      if (cmd is AssertRequiresCmd)
      {
        AssertRequiresCmd assertCmd = (AssertRequiresCmd) cmd;
        Contract.Assert(assertCmd != null);
        CallCounterexample cc = new CallCounterexample(options, assumedCmds, trace, augmentedTrace, assertCmd, errModel, mvInfo,
          context, split, assertCmd.Checksum);
        return cc;
      }
      else if (cmd is AssertEnsuresCmd)
      {
        AssertEnsuresCmd assertCmd = (AssertEnsuresCmd) cmd;
        Contract.Assert(assertCmd != null);
        ReturnCounterexample rc = new ReturnCounterexample(options, assumedCmds, trace, augmentedTrace, assertCmd, transferCmd, errModel, mvInfo,
          context, split, cmd.Checksum);
        return rc;
      }
      else
      {
        AssertCounterexample ac = new AssertCounterexample(options, assumedCmds, trace, augmentedTrace, (AssertCmd) cmd, errModel, mvInfo, context, split);
        return ac;
      }
    }
      ...
      Block implEntryBlock, Dictionary<TransferCmd, ReturnCmd> gotoCmdOrigins)
    {
      ...
      if (assrt is AssertRequiresCmd)
      {
        ...
        cc = new CallCounterexample(options, cex.AssumedCmds, cex.Trace, cex.AugmentedTrace, aa, cex.Model, cex.MvInfo, cex.Context, cex.ProofRun, aa.Checksum);
      }
      else if (assrt is AssertEnsuresCmd && cex is ReturnCounterexample)
      {
        ...
        cc = new ReturnCounterexample(options, cex.AssumedCmds, reconstructedTrace ?? cex.Trace, cex.AugmentedTrace, aa, returnCmd ?? oldCex.FailingReturn,
          cex.Model, cex.MvInfo, cex.Context, cex.ProofRun, aa.Checksum);
      }
      else
      {
        cc = new AssertCounterexample(options, cex.AssumedCmds, cex.Trace, cex.AugmentedTrace, assrt, cex.Model, cex.MvInfo, cex.Context, cex.ProofRun);
      }
    ...
    VCExpr P_LetVC(List<Block> blocks,
      HashSet<string> constantsAssumed,
      VCExpr controlFlowVariableExpr,
      ControlFlowIdMap<Absy> absyIds,
      ProverContext proverCtxt,
      ...
      out int assertionCount)
    {
      ...
      if (gotocmd != null)
      {
        ...
        {
          Contract.Assert(successor != null);
          VCExpr c = P_DagVC(successor, constantsAssumed, controlFlowVariableExpr, absyIds, blockEquations, proverCtxt, out var ac);
          assertionCount += ac;
          if (controlFlowVariableExpr != null)
          {
            VCExpr controlFlowFunctionAppl = gen.ControlFlowFunctionApplication(controlFlowVariableExpr,
