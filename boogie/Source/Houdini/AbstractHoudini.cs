...
namespace Microsoft.Boogie.Houdini
{
    ...
    public class AbsHoudini: ProofRun
    {
        public string Description { get; set; }
        public List<Counterexample> Counterexamples { get; } = new();
        HoudiniOptions Options  { get; }
        ...
        public AbsHoudini(Program program, HoudiniOptions options, IAbstractDomain defaultElem)
        {
            ...
            {
                if(QKeyValue.FindBoolAttribute(func.Attributes, "existential"))
                {
            ...
            foreach(var func in existentialFunctions)
            {
                Console.WriteLine($"Existential func: {func}");
            }

            //if (Options.ProverKillTime > 0)
            //    Options.ProverOptions.Add(string.Format("TIME_LIMIT={0}", Options.ProverKillTime));

            Inline();
            // this.vcgen = new VCGen(program, Options.SimplifyLogFilePath, Options.SimplifyLogFileAppend, new List<Checker>());
            var checkerPool = new CheckerPool(Options);
            this.vcgen = new VCGen(program, checkerPool);
            // this.prover = ProverInterface.CreateProver(program, Options.SimplifyLogFilePath, Options.SimplifyLogFileAppend, -1);
            this.prover = ProverInterface.CreateProver(Options, program, Options.ProverLogFilePath,
        Options.ProverLogFileAppend, Options.TimeLimit);
            this.proverTime = TimeSpan.Zero;
            this.numProverQueries = 0;

            program.TopLevelDeclarations.OfType<Implementation>()
                .Where(impl => !impl.IsSkipVerification(Options))
        ...
        public async Task<VCGenOutcome> ComputeSummaries()
        {
            ...
            while (worklist.Any())
            {
                ...
                env.Typecheck(new TypecheckingContext(null, Options));
                ...
                prover.Push();
                prover.FlushAxiomsToTheoremProver();
                ProverInterface.Outcome proverOutcome = await prover.Check(impl, vc, handler, Options.ErrorLimit, CancellationToken.None);
        ...
        {
            var tt = new TokenTextWriter(Console.Out, Options);
        ...
        public async Task<HashSet<string>> HandleCounterExample(string impl, Counterexample error)
        {
            ...
            var cex = await ExtractState(impl, error);
        ...
        private Task<List<Tuple<string, List<Model.Element>>>> ExtractState(string impl, Counterexample error)
        {
        ...
        private async Task<List<Tuple<string, List<Model.Element>>> > ExtractState(string impl, Expr expr, Model model)
        {
            ...
            foreach (var tup in funcsUsed.Where(t => t.Item2 == null))
            {
                ...
                prover.Context.BoogieExprTranslator.Translate(func.Args).Iter(async ve => vals.Add(await getValue(ve, model)));
            ...
            foreach (var tup in funcsUsed.Where(t => t.Item2 != null))
            {
                ...
                var reporter = new AbstractHoudiniErrorReporter(Options);
                ...
                ProverInterface.Outcome proverOutcome = await prover.Check(impl, ve, reporter, Options.ErrorLimit, CancellationToken.None);
                ...
                foreach (var funcArg in func.Args.OfType<Expr>())
                {
                    ...
                    vals.Add(await getValue(prover.Context.BoogieExprTranslator.Translate(arg), model));
                }
                ret.Add(Tuple.Create(funcName, vals));
        ...
        private async Task<Model.Element> getValue(VCExpr arg, Model model)
        {
            ...
            var val = await prover.Evaluate(arg);
        ...
        class AbsHoudiniCounterexampleCollector : VerifierCallback
        {
            ...
            public AbsHoudiniCounterexampleCollector(AbsHoudini container, CoreOptions.ProverWarnings printProverWarnings): base(printProverWarnings)
            {
            ...
            public async override Task OnCounterexample(Counterexample ce, string reason)
            {
                numErrors++;
                errors.Add(ce);

                funcsChanged.UnionWith( await
        ...
        private void GenVC(Implementation impl)
        {
            ...
            ControlFlowIdMap<Absy> label2absy = null;
            var collector = new AbsHoudiniCounterexampleCollector(this, Options.PrintProverWarnings);
            ...
            var gotoCmdOrigins = vcgen.PassifyImpl(new ImplementationRun(impl, Console.Out), out mvInfo);
            ...
            VCExpr controlFlowVariableExpr = true ? null : gen.Integer(Microsoft.BaseTypes.BigNum.ZERO);

            var vcexpr = vcgen.GenerateVC(impl, controlFlowVariableExpr, label2absy, prover.Context);
            if (!true)
            {
            ...
            // if (Options.vcVariety == CommandLineOptions.VCVariety.Local)
            ...
            // else
                // handler = new VCGen.ErrorReporter(Options, gotoCmdOrigins, label2absy, impl.Blocks, vcgen.incarnationOriginMap, collector, mvInfo, prover.Context, program);
            this.Description = impl.Name;
            this.Counterexamples.Clear();
            handler = new VCGen.ErrorReporter(Options, gotoCmdOrigins, label2absy, impl.Blocks, vcgen.debugInfos, collector, mvInfo, prover.Context, program, this);
        ...
        private void Inline()
        {
            ...
            var callGraph = Program.BuildCallGraph(Options, program);
        ...
        private Graph<Implementation> BuildCallGraph()
        {
            ...
            foreach (Declaration decl in program.TopLevelDeclarations)
            {
                ...
                if (impl == null || impl.IsSkipVerification(Options)) continue;
                callGraph.AddSource(impl);
    ...
    public class AbstractHoudini
    {
        ...
        public AbstractHoudini(Program program, HoudiniOptions options)
        {
            this.Options = options;
            this.program = program;
            this.impl2VC = new Dictionary<string, VCExpr>();
            ...
            if (Options.TimeLimit > 0)
                Options.ProverOptions.Add(string.Format("TIME_LIMIT={0}", Options.TimeLimit));
            ...
            this.prover = ProverInterface.CreateProver(Options, program, Options.ProverLogFilePath,
        Options.ProverLogFileAppend, Options.TimeLimit);
            this.reporter = new AbstractHoudiniErrorReporter(Options);

            this.proverTime = TimeSpan.Zero;
            this.numProverQueries = 0;

            // if (Options.AbstractHoudini == "0")
            //     UseBilateralAlgo = false;
        }

        public async void computeSummaries(ISummaryElement summaryClass)
        {
            ...
            // Options.UseSubsumption = CommandLineOptions.SubsumptionOption.Never;
            ...
            while (worklist.Any())
            {
                ...
                var changed = await ProcessImpl(impl);
        ...
        public async Task<HashSet<string>> GetPredicates()
        {
            var ret = new HashSet<string>();
            // prover = ProverInterface.CreateProver(program, Options.SimplifyLogFilePath, Options.SimplifyLogFileAppend, -1);
            this.prover = ProverInterface.CreateProver(Options, program, Options.ProverLogFilePath,
        Options.ProverLogFileAppend, Options.TimeLimit, -1);
            foreach (var tup in impl2Summary)
            {
                var s = tup.Value as PredicateAbs;
                if (s == null) continue;
                ret.UnionWith(await s.GetPredicates(program, prover.VCExprGen, prover));
                // debug output
        ...
        private void ProduceWitness(Dictionary<string, Implementation> copy)
        {
            ...
            using (var wt = new TokenTextWriter(WitnessFile, Options))
            {
                program.Emit(wt);
            }
            ...
            if (Options.Trace) Console.WriteLine("Witness written to {0}", WitnessFile);
        }
        ...
        private async Task<bool> ProcessImpl(Implementation impl)
        {
            var ret = false;
            var gen = prover.VCExprGen;
            ...
            while (true)
            {
                ...
                ProverInterface.Outcome proverOutcome = await prover.Check(impl.Name, vc, reporter, Options.ErrorLimit, CancellationToken.None);
        ...
        {
            ModelViewInfo mvInfo;
            var label2absy = new ControlFlowIdMap<Absy>();
    ...
    {
        public static HoudiniOptions Options {get; private set;}
        ...
        public static void Initialize(Program program, HoudiniOptions options)
        {
            Options = options;
        ...
        private static Tuple<PredicateAbsDisjunct, Expr> ParseExpr(Expr expr, string procName)
        {
            ...
            {
                if (Options.Trace) Console.WriteLine("Failed to parse {0} (ignoring)", expr);
                // Console.WriteLine("Failed to parse {0} (ignoring)", expr);
                return null;
            }
            ...
            foreach (var atom in atoms)
            {
                ...
                if (index == -1)
                {
                    ...
                    if (index == -1)
                    {
                        if (Options.Trace) Console.WriteLine("Failed to parse {0} (ignoring)", atom);
                        // Console.WriteLine("Failed to parse {0} (ignoring)", atom);
                        return null;
                    }
                    else
                    {
        ...
        public static async void FindUnsatPairs(VCExpressionGenerator gen, ProverInterface prover)
        {
                ...
                foreach (var proc in map.Keys)
                {
                    for (int i = 0; i < 2 * map[proc].Count(); i++)
                    {
                        ...
                        for (int j = 2 * ((i / 2) + 1); j < 2 * map[proc].Count(); j++)
                        {
                            ...
                            if (! await CheckIfUnsat(e1, e2, gen, prover))
                            {
                                cacheNeg.Add(key);
                                continue;
                            }
        ...
        // Is a ^ b UNSAT?
        private static async Task<bool> CheckIfUnsat(Expr a, Expr b, VCExpressionGenerator gen, ProverInterface prover)
        {
            ...
            return await CheckIfUnsat(Expr.And(a, b), gen, prover);
        }

        // Is a UNSAT?
        private static async Task<bool> CheckIfUnsat(Expr a, VCExpressionGenerator gen, ProverInterface prover)
        {
            var gatherLitA = new GatherLiterals();
            gatherLitA.Visit(a);
            ...
            // prover.Check();
            // var outcome = prover.CheckOutcomeCore(new AbstractHoudiniErrorReporter(Options));
            // prover.Pop();
            ProverInterface.Outcome outcome = await prover.Check("Temp", vc1, new AbstractHoudiniErrorReporter(Options), Options.ErrorLimit, CancellationToken.None);
        ...
        public async Task<HashSet<string>> GetPredicates(Program program, VCExpressionGenerator gen, ProverInterface prover)
        {
            var ret = new HashSet<string>();
            ...
            for (int i = 0; i < PostPreds[procName].Count; i++)
            {
                ...
                else
                {
                    foreach (var c in value[i].GetConjuncts())
                    {
                        ...
                        if (await CheckIfUnsat(Expr.And(freeSummary, Expr.Not(s)), gen, prover))
                            continue;
                        ret.Add(s.ToString());
                    }
                }
    ...
    {
        public Model model;
        HoudiniOptions Options;

        public AbstractHoudiniErrorReporter(HoudiniOptions options): base(options)
        {
            this.Options = options;
            model = null;
        }

        public override async Task OnModel(IList<string> labels, Model model, ProverInterface.Outcome proverOutcome)
        {
            Debug.Assert(model != null);
            if (Options.PrintErrorModel >= 1) model.Write(Console.Out);
            this.model = model;
        }
    }
