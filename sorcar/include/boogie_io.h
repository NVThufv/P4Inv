...
namespace horn_verification
{
	...
	public:
		...
		/**
		 * Reads an interval file.
		 *
		 * @param filename The name of the file to read from
		 *
		 * @return Returns the intervals read from file
		 */
		static std::vector<std::pair<unsigned, unsigned>> read_intervals_file(const std::string & filename);
		
		
		/**
		 * Writes a set of conjunctions to JSON file.
		 *
		 * @param conjunctions The list of conjunctions to serialize
		 * @param filename The name of the file to write to
		 */
		static void write_JSON_file(const attributes_metadata & metadata, const std::vector<std::set<unsigned>> & conjunctions, const std::string & filename);
